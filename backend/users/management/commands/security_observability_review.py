...
class Command(BaseCommand):
    ...
    def add_arguments(self, parser):
        parser.add_argument(
            '--strict-prod',
            action='store_true',
            help='Enforce production-only secure defaults even when DEBUG=True.',
        )

    def handle(self, *args, **options):
        strict_prod = options.get('strict_prod', False)
        enforce_prod_secure = strict_prod or not bool(getattr(settings, 'DEBUG', False))

        checks = [
            ('SECURE_SSL_REDIRECT', (not enforce_prod_secure) or bool(getattr(settings, 'SECURE_SSL_REDIRECT', False))),
            ('SESSION_COOKIE_SECURE', (not enforce_prod_secure) or bool(getattr(settings, 'SESSION_COOKIE_SECURE', False))),
            ('CSRF_COOKIE_SECURE', (not enforce_prod_secure) or bool(getattr(settings, 'CSRF_COOKIE_SECURE', False))),
            ('SECURE_HSTS_SECONDS>0', int(getattr(settings, 'SECURE_HSTS_SECONDS', 0)) > 0),
            ('CORS_ALLOW_ALL_ORIGINS=False', (not enforce_prod_secure) or not bool(getattr(settings, 'CORS_ALLOW_ALL_ORIGINS', True))),
